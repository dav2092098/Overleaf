...
\title{Use of Computational Neural Networks for Rapid Bacterial Strain Identification Using Deinococcus aquaticus Isolates Obtained from Biofilm Samples}
...
\author{By: David Ayodele, Stacy Scholz-Ng, and Chad Albert \\ Supervised by: Dr. James Tuohy}

\address{Glendale Community College, Glendale, Arizona, USA}

\begin{abstract}
%% Text of abstract
The spectra produced from cell protein extracts using Matrix-assisted laser desorption/ionization – Time of Flight (MALDI-TOF) mass spectrometry has been shown to contain characteristic fingerprints of specific organisms \textsuperscript{1}. A mapping of 20 strains of Deinococcus Aquaticus\textsuperscript{2} to their MALDI-TOF spectra is demonstrated using a computational neural network (machine learning) algorithm. The algorithm was written in the Python 3 language using the Jupyter/IPython application environment. The MALDI-TOF data used comes from 21 D. aquaticus isolates obtained from biofilm samples. Using the known strain corresponding to each spectrum, a training dataset for the algorithm was created. The training dataset data is used to "teach" the algorithm which strain corresponds to which spectrum. In order to properly train the algorithm, the training data is fed into machine learning functions created using the Keras open-source neural network library for Python. Keras uses the TensorFlow open-source mathematics library for Python as a back-end. Tensor Flow is a computational library allowing for greater ease in defining, optimizing, and evaluating mathematical expressions involving multi-dimensional arrays. A predictive model (akin to a curve in curve fitting) was generated by Keras using the spectral data training sets as inputs. Model (fitting) parameters of note include the use of ReLU (Rectifier Linear Unit as the model activation function, and categorical cross entropy as the loss function. The the loss function represents the objective that the model will try to minimize during fitting while the activation function represents the means by which initial fitting will be attempted (ReLU is analogous to half-wave rectification in electrical signal processing). Prior to use, the MALDI-TOF spectral data was converted to array (vector) data using the Numpy open-source numerical analysis library for Python. Exactly 32 iterations were used in the modeling (fitting) process. A series of spectra of unspecified origin were then passed into the model prediction function. The resulting Keras predictions were then analyzed using standard error analyses. The technique described is used in hopes of arriving at a reasonably reliable method of rapid bacterial strain identification using spectral data in conjunction with machine learning software. 
\end{abstract}

\begin{keyword}
Biotechnology \sep MALDI-TOF \sep Deinococcus Aquaticus\sep Computational Neural Networks \sep Machine Learning \sep TensorFlow algorithms, Python algorithms
...
\pagebreak
Error analysis revealed the Keras predictions to be XX.X\% accurate with confidence interval XX.X

\section{The First Section}
The exact categorical cross entropy function is as follows:
